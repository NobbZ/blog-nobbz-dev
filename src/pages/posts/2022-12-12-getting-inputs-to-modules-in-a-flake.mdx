...
There is a frequently re-occuring scenario in the various community chats and
...
installed via the `system.environmentPackages`.
...
<NoteBox>
	For home-manager it is done the same way, though the argument name is `extraSpecialArgs`.
</NoteBox>
...
<NoteBox>
	You can't use modules from this in an `imports` list, that would produce an infinite recursion
	error.
</NoteBox>
...
Personally I am an offender of overlays, and use them only when there isn't
